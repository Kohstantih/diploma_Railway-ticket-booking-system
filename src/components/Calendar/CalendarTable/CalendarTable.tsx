...
export default function CalendarTable({
  ...
  const checkDate = useCallback(
    (date: number) => {
      setDateChecked(date);
      setDate(
        `${`0${date}`.slice(-2)}.${`0${month + 1}`.slice(-2)}.${`${year}`.slice(
          -2
        )}`
      );
    },
  ...
  useEffect(() => {
    ...
    const firstDayNumber =
      dateBeginMonth.getDay() === 0 ? 7 : dateBeginMonth.getDay();
    ...
    setList((value) => [...value, <tr key={'firstWeek'}>{firstWeek}</tr>]);
    ...
    if (thisMonthLength) {
      for (let u = 0; u < thisMonthLength / 7; u += 1) {
        ...
        if (otherWeek.length === 7) {
          setList((value) => [...value, <tr key={`week${u}`}>{otherWeek}</tr>]);
        }
      }
    }
    ...
    if (thisMonthLength && lastValue < thisMonthLength) {
      ...
      setList((value) => [...value, <tr key={'lastWeek'}>{lastWeek}</tr>]);
    }

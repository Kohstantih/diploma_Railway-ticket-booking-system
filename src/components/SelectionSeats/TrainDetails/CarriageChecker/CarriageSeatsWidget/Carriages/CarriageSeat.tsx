...
export default function CarriageSeat({ freeSeats }: { freeSeats: number[] }) {
      ...
      if (freeSeats.includes(value)) {
        return (
          <p
            ...
            className={`seat__number seat__number_seats seat_free${
        ...
        return (
          <p key={`seat${value}`} className={'seat__number seat__number_seats seat_disabled'}>
            {value}
          </p>
        );
      }
  ...
  useEffect(() => {
    ...
    for (let i = 34; i < 61; i += 2) {
      thirdRow.push(createNumberElement(i));
    }

    const fourthRow: JSX.Element[] = [];

    for (let i = 33; i < 64; i += 2) {
      if (i === 63) {
        fourthRow.push(createNumberElement(62));
        break;
      }
      fourthRow.push(createNumberElement(i));
    }

    result.push(
      <div key={'section2'} className="seats__section">
        <div className="seats__row row_short">{thirdRow}</div>
        <div className="seats__row">{fourthRow}</div>
      </div>
    );
  ...
  return (
    <div className="carriage-seat__container carriage-seat__container_seat">
